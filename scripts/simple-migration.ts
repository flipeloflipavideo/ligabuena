...
async function main() {
  console.log('🚀 Simple migration to Neon PostgreSQL...')
  ...
  try {
    ...
    // Create simple matches without goals for now
    console.log('⚽ Creating matches...')
    for (const league of leagues) {
      const teams = await db.team.findMany({ where: { leagueId: league.id } })
      
      // Create 3 matches per league (simplified)
      for (let i = 0; i < 2; i++) {
        const match = await db.match.create({
          data: {
            ...
            matchDate: new Date(`2024-10-${15 + i}T15:00:00`),
      ...
      console.log(`✅ Created 2 matches for ${league.name}`)
    }
    ...
    console.log('🎉 Simple migration to Neon PostgreSQL completed successfully!')
